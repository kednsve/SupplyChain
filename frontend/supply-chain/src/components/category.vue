...
import { useCategoryStore } from '@/stores/CategoryTable.ts'
...
const { categoryData, total, current } = storeToRefs(categoryStore)
const loading = ref(true)

let tableData = ref(categoryData.value)
onMounted(async () => {
  if (tableData.value.length === 0) {
    if (categoryData.value.length === 0) {
      tableData.value = await categoryStore.fetchCategories(current.value, pageSize.value)
    }
    loading.value = false
  } else {
    loading.value = false
...
const searchById = async () => {
  ...
  tableData.value = [await categoryStore.fetchCategory(Number(searchId.value))]
}
const searchCancel = async () => {
  current.value = 1
  tableData.value = categoryStore.getTableData()
  usePage.value = true
  searchId.value = ''
...
const BSReset = async () => {
  searchName.value = ''
  tableData.value = await categoryStore.fetchCategories(1, pageSize.value)
}

const BSCommit = async () => {
  loading.value = true
  tableData.value = await categoryStore.fetchCategories(1, pageSize.value, searchName.value || null)
  loading.value = false
}
// 批量删除
...
const handleSelectionChange = (val: Category[]) => {
  selectList.value = val.map((category) => category.id)
}
const mutiDel = async () => {
  if (selectList.value.length === 0) {
    operationDialog('error', '请勾选要删除的信息')
  } else {
    const code = await categoryStore.fetchDeleteCategory(selectList.value)
    if (code === 200) {
      await categoryStore.fetchCategories(current.value, pageSize.value)
      tableData.value = categoryStore.getTableData()
...
const openDialog = (id: number, type: 'update' | 'delete') => {
  ...
  const found = categoryData.value.find((item) => item.id === id)
...
//修改数据
const execUpd = async () => {
  const updCategory = formData.value as Category
  const code = await categoryStore.fetchUpdateCategory(updCategory)
  if (code === 200) {
    const idx = categoryData.value.findIndex((item) => item.id === updCategory.id)
    if (idx !== -1) {
      categoryData.value[idx] = { ...updCategory }
      categoryStore.setTableData(categoryData.value)
    }
...
// 删除数据
const execDel = async () => {
  const delCategory = formData.value as Category
  const code = await categoryStore.fetchDeleteCategory([delCategory.id])
  if (code === 200) {
    tableData.value = await categoryStore.fetchCategories(current.value, pageSize.value)
    operationDialog('success', '删除成功')
    dialogVisible.value = false
...
<template>
  ...
  >
    ...
    <el-table-column label="Name" property="name" width="1000" />
