...
import { useDepartmentStore } from '@/stores/DepartmentTable.ts'
...
const { departmentData, total, current } = storeToRefs(departmentStore)
const loading = ref(true)

let tableData = ref(departmentData.value)
onMounted(async () => {
  if (tableData.value.length === 0) {
    if (departmentData.value.length === 0) {
      tableData.value = await departmentStore.fetchDepartments(current.value, pageSize.value)
    }
    loading.value = false
  } else {
    loading.value = false
...
const searchById = async () => {
  ...
  tableData.value = [await departmentStore.fetchDepartment(Number(searchId.value))]
}
const searchCancel = async () => {
  current.value = 1
  tableData.value = departmentStore.getTableData()
  usePage.value = true
  searchId.value = ''
...
const BSReset = async () => {
  searchName.value = ''
  tableData.value = await departmentStore.fetchDepartments(1, pageSize.value)
}

const BSCommit = async () => {
  loading.value = true
  tableData.value = await departmentStore.fetchDepartments(
    1,
    pageSize.value,
...
const handleSelectionChange = (val: Department[]) => {
  selectList.value = val.map((department) => department.id)
}
const mutiDel = async () => {
  if (selectList.value.length === 0) {
    operationDialog('error', '请勾选要删除的信息')
  } else {
    const code = await departmentStore.fetchDeleteDepartment(selectList.value)
    if (code === 200) {
      await departmentStore.fetchDepartments(current.value, pageSize.value)
      tableData.value = departmentStore.getTableData()
...
//修改数据
const execUpd = async () => {
  const updDepartment = formData.value as Department
  const code = await departmentStore.fetchUpdateDepartment(updDepartment)
  if (code === 200) {
    const idx = departmentData.value.findIndex((item) => item.id === updDepartment.id)
    if (idx !== -1) {
      departmentData.value[idx] = { ...updDepartment }
      departmentStore.setTableData(departmentData.value)
    }
...
// 删除数据
const execDel = async () => {
  const delDepartment = formData.value as Department
  const code = await departmentStore.fetchDeleteDepartment([delDepartment.id])
  if (code === 200) {
    tableData.value = await departmentStore.fetchDepartments(current.value, pageSize.value)
    operationDialog('success', '删除成功')
    dialogVisible.value = false
